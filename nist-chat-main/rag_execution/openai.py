...
async def send_get_request(url, key=None, user: UserModel = None):
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout, trust_env=True, connector=no_ssl_connector) as session:
...
async def proxy(path: str, request: Request, user=Depends(get_verified_user)):
    ...
    try:
        no_ssl_connector = aiohttp.TCPConnector(ssl=False)
        session = aiohttp.ClientSession(trust_env=True, connector=no_ssl_connector)
        r = await session.request(
